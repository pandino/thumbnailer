        ...
        if pending_filename == filename then
            ...
            confirmation_timer = mp.add_timeout(CONFIRMATION_TIMEOUT, function()
                if pending_deletion and pending_filename == filename then
                    mp.osd_message(config.icons.error .. " Deletion cancelled (timeout)", 2)
...
msg.info("Enhanced delete movie script loaded. Press 'd' twice within 3 seconds to mark for deletion. Endpoint: " .. API_ENDPOINT)
